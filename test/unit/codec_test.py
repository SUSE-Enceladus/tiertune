...
from tiertune.exceptions import TierTuneDecodingError
...
class TestCodec:
    ...
    @patch('tiertune.codec.Codec._wrapped_decode')
    def test_decode_ascii_failure(self, mock_decode):
        ...
        def mocked_decode(literal, encoding, error_handling_schema):
            if encoding:
                ...
                raise TierTuneDecodingError('ascii decoding failure')
    ...
    def test_decode_utf8_failure(self, mock_decode):
        def mocked_decode(literal, encoding, error_handling_schema):
            if encoding:
                raise TierTuneDecodingError('utf-8 decoding failure')
            else:
                raise TierTuneDecodingError('ascii decoding failure')

        mock_decode.side_effect = mocked_decode
        with raises(TierTuneDecodingError):
            with self._caplog.at_level(logging.WARNING):
